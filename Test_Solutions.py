...
confirmList = [element.strip() for element in newestList]
confirmList.remove("")
...
print(color_count)
#or do:
#color_count = counter(confirmList)
...
mean = round(sum(color_count_values) / len(color_count_values))
...
mean_deviations = [(number-mean)**2 for number in color_count_values]
...
print("\nThe probability of picking RED at random from the colors is: ", float(color_count["RED"]/sum(color_count_values)), "\n")
